        ...
        <div class="row">
          <div class="col-12">
            <div class="page-title-box">
              <h4 class="page-title">Identity Proof</h4>
          ...
          <div class="col-md-12">
            <div class="card">
              <div class="card-body">
                ...
                data-overlaySpeed="100" data-overlayColor="#36404a"><i class="fa fa-plus-circle"></i> Add Identity Proof</a>
                 ...
                 <!--      <th>Card ID</th> -->
                  ...
                  <tbody>
                    ...
                    <tr class="gradeX" style="background-color: white">
                      ...
                      <!-- <td> <?php echo $row["card_id"]; ?>
                      </td> -->
                      <td>
                        <span class="badge badge-pill badge-info">
                          <?php
                          $sqlI = "SELECT * FROM employee WHERE identity_id = $id ";
                          $resultI = query($sqlI);
                          confirm($resultI);
                          $iI = 0;
                      ...
                      <td class="actions">
                        <?php if($status == 0){ ?>
                        <a href="page-identity-card.php?block=<?php echo $id?>" onclick="return confirm('Are you sure ?')" title="Block" class="hidden on-editing save-row"><i class="fa fa-ban" style="color: red;"></i></a>
                        <?php } ?>
                        <?php if($status == 1){ ?>
                        <a href="page-identity-card.php?accept=<?php echo $id?>" onclick="return confirm('Are you sure ?')" title="Approve"  class="hidden on-editing cancel-row"><i class="fa fa-check"></i></a>
                        <a href="page-identity-card.php?delete=<?php echo $id?>" onclick="return confirm('Are you sure ?')" title="Delete" class="on-default remove-row"><i class="fa fa-trash-o"></i></a>
                        <?php } ?>
                      </td>
                    </tr>
                    <?php
...
if (isset($_GET['block'])) {
    $block_id = $_GET['block'];
    $quer     = "UPDATE identity SET status = 1 WHERE id = '$block_id'";
    query($quer);
...
if (isset($_GET['accept'])) {
    ...
    $quer      = "UPDATE identity SET status = 0 WHERE id = '$accept_id'";
    query($quer);
    header("location: page-identity-card.php");
...
if (isset($_GET['delete'])) {
    ...
    $sql       = "DELETE FROM identity WHERE id ='$delete_id'";
    query($sql);
    header("location: page-identity-card.php");
    set_message("<div class='alert alert-danger'>
...
<div id="custom-modal" class="modal-demo">
  ...
  <h4 class="custom-modal-title">Add new Identity Proof</h4>
